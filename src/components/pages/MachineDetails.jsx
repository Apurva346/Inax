...
// const MachineDetails = () => {
//   const { id } = useParams(); // URL मधून येणारा parameter (उदा. closed-body-fiber-laser-cutting-machine)
  
//   useEffect(() => {
//     window.scrollTo(0, 0);
//   }, [id]);

//   // machinesData चा nested object मधून अचूक मशीन शोधण्याचे लॉजिक
//   const machine = React.useMemo(() => {
//     if (!machinesData || !id) return null;

//     // १. सर्व कॅटेगरीजमधील (Laser, CNC, Bending) मशिन्स एकत्र करून flat array बनवतो
//     const allMachines = Object.values(machinesData).flat();

//     // २. URL स्लगला डेटा मधील 'id' किंवा 'name' सोबत मॅच करतो
//     return allMachines.find((m) => {
//       // मशीनच्या नावापासून सुरक्षित URL स्लग बनवणे
//       const dataSlug = m.name
//         ? m.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')
//         : '';
      
//       // जर थेट m.id मॅच झाला OR नावाचा बनवलेला स्लग मॅच झाला तर रिटर्न करेल
//       return m.id === id || dataSlug === id;
//     });
//   }, [id]);

//   // जर मशीन सापडले नाही तर ही एरर स्क्रीन दिसेल
//   if (!machine) {
//     return (
//       <div className="py-32 text-center">
//         <h2 className="text-3xl font-bold text-red-700 uppercase tracking-wider">Machine Not Found</h2>
//         <p className="text-gray-500 mt-3 text-sm">
//           Requested URL Path: <span className="font-mono text-black bg-gray-100 px-2 py-1 rounded border border-gray-200">{id}</span>
//         </p>
//         <Link to="/" className="mt-8 inline-block bg-gray-900 text-white px-8 py-3 rounded font-bold uppercase text-xs tracking-widest hover:bg-red-700 transition-colors">
//           Back to Home
//         </Link>
//       </div>
//     );
...
//         .faq-box { background: #f3f4f6; padding: 1.5rem; border-radius: 12px; margin-top: 1rem; border-left: 5px solid #003366; margin-bottom: 1rem; }
//       `}</style>

//       {/* Main Container */}
//       <div className="flex flex-col lg:flex-row lg:items-start gap-12">
        
//         {/* 1. Header Section (Title & Breadcrumbs) - Mobile Only */}
//         <div className="w-full lg:hidden order-1">
//           <nav className="flex mb-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
//             <Link to="/" className="hover:text-orange-500">Home</Link>
//             <span className="mx-2">/</span>
...
//         {/* 2. Image Section */}
//         <div className="w-full lg:w-[45%] lg:sticky lg:top-24 order-2 lg:order-1">
//           <div className="relative rounded-2xl overflow-hidden shadow-2xl border-b-[10px] border-[#003366] bg-gray-100 p-6 flex items-center justify-center">
//             <img 
//               src={machine.image} 
//               alt={machine.name} 
//               className="w-full h-[400px] object-contain group-hover:scale-105 transition-transform duration-500" 
//               onError={(e) => {
//                 e.target.src = 'https://placehold.co/600x450?text=Image+Coming+Soon';
//               }}
//             />
//           </div>
...
//           {/* लाँग डिस्क्रिप्शन रेंडरिंग (जर HTML फॉरमॅटमध्ये असेल तर) */}
//           <div className="client-html">
//             {machine.longDescription ? (
//               <div dangerouslySetInnerHTML={{ __html: machine.longDescription }} />
//             ) : (
//               <div>
//                 {/* Fallback जर लाँग डिस्क्रिप्शन नसेल तर साधा मजकूर दाखवण्यासाठी */}
//                 <p className="text-lg text-gray-600 leading-relaxed mb-6">
//                   {machine.description || machine.des}
//                 </p>
                
//                 {/* टेक्निकल स्पेसिफिकेशन्स */}
//                 {machine.specs && machine.specs.length > 0 && (
//                   <div className="mt-8">
//                     <h3 className="text-sm font-black uppercase mb-4 tracking-wider text-gray-400 border-b pb-2">
//                       Technical Specs
//                     </h3>
...
const MachineDetails = () => {
  const { id } = useParams(); // URL मधून येणारा slug
  
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [id]);

  // नवीन Nested Object मधून मशिन्स शोधण्याचे शक्तिशाली लॉजिक
  const machine = React.useMemo(() => {
    if (!machinesData || !id) return null;

    let allMachines = [];

    // सर्व कॅटेगरीज आणि सब-कॅटेगरीजमधून items (मशीन्स) बाहेर काढणे
    Object.values(machinesData).forEach((categoryObj) => {
      if (categoryObj.subcategories) {
        Object.values(categoryObj.subcategories).forEach((subCat) => {
          if (subCat.items && Array.isArray(subCat.items)) {
            allMachines.push(...subCat.items);
          }
        });
      } else if (Array.isArray(categoryObj)) {
        // जर जुना एरे फॉरमॅट बॅकअप म्हणून राहिला असेल तर
        allMachines.push(...categoryObj);
      }
    });

    // अचूक मॅचिंग करणे (ID किंवा नावाचा स्लग)
    return allMachines.find((m) => {
      const dataSlug = m.name
        ? m.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')
        : '';
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 lg:py-10">
      ...
      <div className="flex flex-col lg:flex-row lg:items-start gap-12">
        ...
        <div className="w-full lg:hidden order-1">
          ...
          <h3 className="text-2xl font-black text-gray-900 uppercase leading-tight">{machine.name}</h3>
          <div className="w-20 h-2 bg-orange-500 mt-4 mb-2"></div>
        </div>
        ...
        <div className="w-full lg:w-[45%] lg:sticky lg:top-24 order-2 lg:order-1">
          <div className="relative rounded-2xl overflow-hidden shadow-2xl border-b-[10px] border-[#003366] bg-gray-100 p-6 flex items-center justify-center">
            <img 
              ...
              alt={machine.name} 
              className="w-full h-[400px] object-contain" 
              onError={(e) => { e.target.src = 'https://placehold.co/600x450?text=Image+Coming+Soon'; }}
            />
          </div>
          <div className="mt-8 flex flex-col sm:flex-row gap-4">
            <Link to="/contact" className="flex-1" onClick={() => window.scrollTo(0, 0)}>
        ...
        <div className="w-full lg:w-[55%] flex flex-col order-3 lg:order-2">
          <div className="hidden lg:block">
            ...
            <h3 className="text-4xl font-black text-gray-900 uppercase leading-tight">{machine.name}</h3>
            <div className="w-20 h-2 bg-orange-500 my-8"></div>
          </div>
          ...
          <div className="client-html">
            {machine.longDescription ? (
              ...
              <div>
                <p className="text-lg text-gray-600 leading-relaxed mb-6">{machine.description || machine.des}</p>
                {machine.specs && machine.specs.length > 0 && (
                  <div className="mt-8">
                    <h3 className="text-sm font-black uppercase mb-4 tracking-wider text-gray-400 border-b pb-2">Technical Specs</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                      {machine.specs.map((spec, index) => (
                        <div key={index} className="flex items-center gap-2 bg-gray-50 p-4 rounded border-l-4 border-[#003366] shadow-sm">
                          <span className="text-gray-800 font-bold text-[13px] uppercase">{spec}</span>
